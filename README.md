...
The repo ships a [`render.yaml`](render.yaml) blueprint for the REST API.

1. Push this repository to GitHub (the blueprint targets the `backend/` subdirectory).
2. Render dashboard → **New → Blueprint** → pick the repo. Render reads `render.yaml` and creates the
   `mdds-api` web service (root dir `backend`, health check `/api/health`).
3. In the service's **Environment** tab set:
   - `MONGODB_URI` — your Atlas connection string (e.g. `mongodb+srv://user:pass@cluster/mdss?retryWrites=true&w=majority`)
   - `JWT_SECRET` — auto-generated by the blueprint; copy it if you need it elsewhere
4. **MongoDB Atlas → Network Access → Add IP Address → `0.0.0.0/0`** (Render egress IPs are dynamic), and
   ideally create a dedicated least-privilege database user for Render.
5. Deploy. Verify: `https://mdds.onrender.com/api/health` must return
   `{"status":"ok","database":"connected"}`.

Without the blueprint, configure the service manually:
